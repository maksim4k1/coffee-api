...
from utils.main import get_token_expires, check_email, check_password, check_token_expires, send_email, check_username

from schemas.users import User, Creds, RegisterUser, UserProfile, Token, LoginUser, ChangeUserUsername, CheckCode, ChangeUserEmail, ChangeUserPassword, ChangeUserPhoto, ChangeUserRole, TokenAuth, LoginUserProfile, SetNewPassword, DeleteUser

from db.users import find_user, add_user, change_user, delete_user
...
class UsersService:
  ...
  def register_user(self, data: RegisterUser) -> LoginUserProfile:
    if check_email(data.auth.email) == False or check_password(data.auth.password) == False:
  ...
  def user_password_recovery_get_email(self, email: str) -> str:
  ...
  def user_password_recovery_get_code(self, data: CheckCode) -> CheckCode:
    ...
    raise HTTPException(status_code=400, detail="Код введен не верно")

  # user_password_recovery_change_password
  def user_password_recovery_change_password(self, data: SetNewPassword) -> bool:
    user: User = find_user(data.email)
    if user != None and user.code == data.code:
      if check_password(data.new_password.new_password) == False:
        raise HTTPException(status_code=400, detail="Некорректный ввод данных")
  ...
  def change_password(self, data: ChangeUserPassword) -> str:
    ...
    if user:
      ...
      elif check_password(data.new_password.new_password) == False:
        raise HTTPException(status_code=400, detail="Некорректный ввод данных")
  ...
  def change_role(self, data: ChangeUserRole) -> str:
    ...
    if admin_user:
      if self._check_admin(admin_user) == False:
        raise HTTPException(status_code=403, detail="ы не можете выполнить это действие")
  ...
  # delete_user
  def delete_user(self, data: DeleteUser) -> str:
    admin_user: User = self._token_auth(TokenAuth(token=data.user.token))
    if admin_user:
      if self._check_admin(admin_user) == False:
        raise HTTPException(status_code=403, detail="ы не можете выполнить это действие")

      deleted_user = find_user(data.id)
      if deleted_user != None:
        delete_user(deleted_user.id)
