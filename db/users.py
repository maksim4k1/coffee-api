...
def find_user(data: str) -> User:
  query = session.query(TableUser).filter(or_(TableUser.id == data, TableUser.email == data, TableUser.token == data))
  user = None
  if query == None: return None
  for item in query:
    user = convert_to_user(item)
    break
  return user
...
def delete_user(user_id: str) -> str:
  query = session.query(TableUser).get(user_id)
  if query == None: return None
